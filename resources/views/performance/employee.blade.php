<x-app-layout>
    ...
    (function () {
        ...
        function updateCycleInfoBanner() {
            ...
            document.getElementById('cycleInfoPeriod').textContent = `${currentCycle.start_date} to ${currentCycle.end_date}`;
            ...
            document.getElementById('cycleInfoSelfDue').textContent = currentCycle.self_review_due_date ?? 'Not set';
            document.getElementById('cycleInfoManagerDue').textContent = currentCycle.manager_review_due_date ?? 'Not set';
        }
        ...
        function objectiveProgress(o) {
            ...
            return o.key_results.reduce((a, kr) => {
                const p = Math.min(100, Math.max(0, (kr.current_value / kr.target_value) * 100 || 0));
        ...
        async function loadObjectives() {
            ...
            container.innerHTML = objectives.map(o => {
                ...
                const krRows = o.key_results.map(kr => {
                    const p = Math.min(100, Math.max(0, (kr.current_value / kr.target_value) * 100 || 0)).toFixed(0);
                    return `
            ...
            container.querySelectorAll('.decline-alignment-btn').forEach(btn => {
                btn.addEventListener('click', async function () {
                    if (!confirm('Decline this alignment? It goes back to the employee as a draft.')) return;
