...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'file_number' => ['nullable', 'string', 'max:255', $uniquePerBusiness('file_number')],
    ...
    public function loginAsEmployee(Request $request, $id)
    {
        ...
        if (!in_array($activeRole, ['business-admin', 'business-hr', 'super-admin'], true)) {
            return RequestResponse::forbidden('You do not have permission to log in as an employee.');
        }

        $employee = Employee::with(['user', 'business'])->find($id);
        if (!$employee) {
            return RequestResponse::badRequest('Employee not found.', 404);
        }

        $targetUser = $employee->user;
        if (!$targetUser) {
            return RequestResponse::badRequest('This employee has no linked user account.');
        }
        ...
        if (!$business) {
            return RequestResponse::badRequest('This employee has no business.');
        }

        session([
            'impersonating_original_user_id' => $adminUser->id,
            'impersonating_original_business_slug' => session('active_business_slug'),
            'impersonating_original_active_role' => session('active_role'),
        ]);
        ...
        activity()
            ...
            ->log('Logged in as employee');

        return RequestResponse::ok('Logged in as employee.', [
            'redirect_url' => route('myaccount.index', ['business' => $business->slug]),
        ]);
    }
    ...
    public function stopImpersonatingEmployee(Request $request)
    {
        ...
        if (!$originalUserId) {
            return RequestResponse::badRequest('Not currently logged in as an employee.');
        }

        $impersonatedUser = $request->user();
        $originalBusinessSlug = session('impersonating_original_business_slug');
        $originalActiveRole = session('impersonating_original_active_role');

        Auth::loginUsingId($originalUserId);

        session([
            'active_business_slug' => $originalBusinessSlug,
            'active_role' => $originalActiveRole,
        ]);
        session()->forget([
            'impersonating_original_user_id',
            'impersonating_original_business_slug',
            'impersonating_original_active_role',
        ]);

        activity()
            ->causedBy($request->user())
            ->performedOn($impersonatedUser)
            ->log('Stopped logging in as employee');

        return RequestResponse::ok('Returned to your account.', [
            'redirect_url' => $originalBusinessSlug
                ? route('business.employees.index', ['business' => $originalBusinessSlug])
                : route('login'),
        ]);
    }
    ...
    public function sendWelcomeEmail(Request $request, $id)
    {
        try {
            ...
            $business = Business::findBySlug(session('active_business_slug'));
            $token = Password::createToken($user);
            $user->notify(new WelcomeEmployeeNotification($user, $token, $business?->company_name));

            return RequestResponse::ok('Welcome email sent successfully.');
        } catch (\Exception $e) {
            Log::error('Error sending welcome email: ' . $e->getMessage());
            return RequestResponse::badRequest('Failed to send welcome email: ' . $e->getMessage());
        }
    }
    ...
    public function view(Request $request)
    {
        try {
            ...
            $departments = \App\Models\Department::where('business_id', $employee->business_id)->orderBy('name')->get(['id', 'name']);
            $jobCategories = \App\Models\JobCategory::where('business_id', $employee->business_id)->orderBy('name')->get(['id', 'name']);

            $view = view('employees._view', compact('employee', 'departments', 'jobCategories'))->render();
            return response()->json([
