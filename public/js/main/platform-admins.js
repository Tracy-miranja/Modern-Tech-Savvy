...
document.getElementById('submitCreatePlatformAdminBtn')?.addEventListener('click', async function () {
    const form = document.getElementById('createPlatformAdminForm');
    ...
    try {
        const response = await requestClient.post(
            `/businesses/${window.currentBusinessSlug}/platform-admins`,
            formData
        );
...
window.revokePlatformAdmin = async function (userId, email) {
    ...
    try {
        const response = await requestClient.post(
            `/businesses/${window.currentBusinessSlug}/platform-admins/${userId}/revoke`,
            {}
        );

        Swal.fire({ icon: 'success', title: 'Revoked', text: response.message });
        document.getElementById(`platform-admin-row-${userId}`)?.remove();
    } catch (error) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Failed to revoke platform admin access.',
        });
    }
};
